...
#Let's consider zero.fare our new subset of data,
...
titles.lookup <- data.frame(Title = c('Col.','Dr.','Mr.','Rev.',
                                      'Mrs.','Dona.','Ms.','Miss.',
                                      'Master.'),
                            New.Title = c(rep('Mr.', 4),
                                          rep('Mrs.',3),
                                          'Miss.',
                                          'Master.'),
...
View(train)
sum(is.na(train$Fare == 0.0)) #this confirms the imputation
...
View(train)
sum(is.na(train$Age)) #this confirms our imputation
...
#Deduction:
#If we set a cutoff for children at let's say Age19 or below, then we have about 15% of children in our data
#If we set a cutoff for old folks at let's say Age55 or more, then we have about 5% of old folks in our data
...
write.csv(x = train, file = 'test.csv', row.names = FALSE)
getwd()
